...
for text_start in text_progress:
    ...
    for vid_idx, vid in enumerate(video_ids):
        ...
        # ✅ 전체 프레임의 중앙값 계산 (batch_size, 1)
        overall_median_similarity = torch.min(video_similarities, dim=1, keepdim=True).values  # (batch_size, 1)

        # ✅ shape 맞추기: (batch_size, num_segments)로 확장
        overall_median_similarity = overall_median_similarity.unsqueeze(-1).expand(-1, 1, segment_avg_similarities.shape[-1])

        non_segment_avg_similarities = torch.where(
            non_segment_lengths > 0,
            non_segment_sums / non_segment_lengths,
            overall_median_similarity 
        )

        # ✅ 최종 Segment Score 계산 (벡터 연산)
        segment_scores = segment_avg_similarities - non_segment_avg_similarities

        # ✅ 가장 높은 segment score의 인덱스 찾기
        max_segment_indices = torch.argmax(segment_scores, dim=-1)  # (batch_size,)
        max_segment_indices = max_segment_indices.view(-1, 1, 1)
        
        # ✅ max_segment_score를 갖는 segment의 평균 유사도 저장
        max_segment_avg_sim_values = segment_avg_similarities.gather(2, max_segment_indices).squeeze(-1)
        
        # ✅ best_video_scores_tensor에 저장
        best_video_scores_tensor[:, vid_idx] = max_segment_avg_sim_values.squeeze(-1) 
